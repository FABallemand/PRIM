...
from models import ScaleHyperpriorTeacher
...
def save_checkpoint(
        state, is_best, path="checkpoint.pth.tar",
...
def make(config):
    ...
    # Create data sets
    if config.dataset == "openImages":
        # train_dataset = OpenImagesDataset(dataset["train"], transform=transform)
        # test_dataset = OpenImagesDataset(dataset["test"], transform=transform)
        pass
    elif config.dataset == "Vimeo90K":
    ...
    # Create model
    model = ScaleHyperpriorTeacher(config.N, config.M).to(device)
...
def train(
    ...
    total_batches = len(train_data_loader) * config.epochs # Total number of batches"
    ...
    for epoch in tqdm(range(config.epochs)):
        ...
        save_checkpoint(
            ...
            is_best,
            config.save_path + "/checkpoint.pth.tar",
            config.save_path + "/checkpoint_best.pth.tar"
        )
...
if __name__ == "__main__":
    save_path = sys.argv[1][0]

    # Experiment configuration
    config = dict(
        dataset="Vimeo90K",
        # dataset="OpenImages",
        architecture="ScaleHyperpriorTeacher",
        N=128,
        M=192,
        epochs=1000,
        batch_size=128,
