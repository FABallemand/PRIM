...
def make(config):
    ...
    teacher_device_r = torch.device("cuda:0")
    teacher_device_d = torch.device("cuda:1")
    student_device = torch.device("cuda:2")
    ...
    # Create models
    url = model_urls["bmshj2018-hyperprior"]["mse"][config.quality_teacher_r]
    state_dict = load_state_dict_from_url(url, progress=False)
    state_dict = load_pretrained(state_dict)
    teacher_model_r = ScaleHyperprior.from_state_dict(state_dict).eval().to(teacher_device_r)
    url = model_urls["bmshj2018-hyperprior"]["mse"][config.quality_teacher_d]
    state_dict = load_state_dict_from_url(url, progress=False)
    state_dict = load_pretrained(state_dict)
    ...
    return (
        teacher_model_r, teacher_model_d, student_model, train_loader,
        validation_loader, test_loader, criterion, optimizer, lr_scheduler
    )


def train_epoch(
        epoch, teacher_model_r, teacher_model_d, student_model,
        criterion, train_loader, optimizer, clip_max_norm=1.0):
    # Set-up
    student_model.train()
    student_device = next(student_model.parameters()).device
    teacher_device_r = next(teacher_model_r.parameters()).device
    teacher_device_d = next(teacher_model_d.parameters()).device

    for i, x in enumerate(train_loader):
        # Load batch
        teacher_x_r = x.to(teacher_device_r)
        teacher_x_d = x.to(teacher_device_d)
        student_x = x.to(student_device)

        # Forward pass
        teacher_output_r = teacher_model_r(teacher_x_r)
        teacher_output_d = teacher_model_d(teacher_x_d)
        student_output = student_model(student_x)
        loss, loss_dict = criterion(student_output,
                                    teacher_output_d["y_hat"].to(student_device),
                                    teacher_output_r["z_hat"].to(student_device), # Hybrid KD
                                    teacher_output_d["x_hat"].to(student_device),
                                    student_x)
...
def validation(
        epoch, data_loader, teacher_model_r, teacher_model_d, student_model,
        criterion):
    # Set-up
    student_model.eval()
    student_device = next(student_model.parameters()).device
...
def train(
        teacher_model_r, teacher_model_d, student_model,
    ...
    for epoch in range(config.epochs):
        ...
        # Train one epoch
        train_epoch(epoch, teacher_model_r, teacher_model_d, student_model,
                    criterion, train_data_loader, optimizer)

        # Validation
        loss = validation(epoch, validation_data_loader, teacher_model_r,
                          teacher_model_d, student_model, criterion)
...
def model_pipeline(config):
    ...
    with wandb.init(project="bmshj2018_hyperprior_hybrid_kd_experiments", config=config):
        # Access config
        config = wandb.config
        print(config)
        ...
        (teacher_model_r, teacher_model_d, student_model, train_loader,
         validation_loader, test_loader, criterion, optimizer, lr_scheduler) = make(config)
        print(teacher_model_r)
        print(teacher_model_d)
        print(student_model)
        print(criterion)

        # Train model
        train(teacher_model_r, teacher_model_d, student_model, train_loader,
              validation_loader, criterion, optimizer, lr_scheduler, config)
...
if __name__ == "__main__":
    ...
    model_pipeline(config) # Check config and wand project name